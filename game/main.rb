  ...
  def button_down(id)
    ...
    when Gosu::KB_RIGHT,Gosu::GP_RIGHT
       if @area1.empty?
        ...
        move_push(PLAYER,WALL,BLOCK,GOAL,VOID,WIN,MOVE,MOVE[0])
       end
    when Gosu::KB_LEFT,Gosu::GP_LEFT
       if @area1.empty?
        ...
        move_push(PLAYER,WALL,BLOCK,GOAL,VOID,WIN,MOVE,MOVE[1])
       end
    when Gosu::KB_UP,Gosu::GP_UP
       if @area1.empty?
        ...
        move_push(PLAYER,WALL,BLOCK,GOAL,VOID,WIN,MOVE,MOVE[2])
       end
    when Gosu::KB_DOWN,Gosu::GP_DOWN
       if @area1.empty?
        ...
        move_push(PLAYER,WALL,BLOCK,GOAL,VOID,WIN,MOVE,MOVE[3])
       end
    when Gosu::KB_R
      arr = []
      File.foreach(Dir.glob("*txt")[@num_level]) { |line| 
